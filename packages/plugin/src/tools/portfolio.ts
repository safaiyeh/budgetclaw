...
export function upsertHolding(db: Database, input: UpsertHoldingInput): PortfolioHoldingRow {
  ...
  const existing = db
    .query('SELECT id FROM portfolio_holdings WHERE account_id = ? AND symbol = ?')
    .get(account_id, symbol) as { id: string } | null;
    ...
    db.run(
      `UPDATE portfolio_holdings
       SET name = COALESCE(?, name),
           ...
           updated_at = ?
       WHERE id = ?`,
      [name ?? null, quantity, price ?? null, value, currency, asset_type ?? null, ts, existing.id]
    );
    return db.query('SELECT * FROM portfolio_holdings WHERE id = ?').get(existing.id) as PortfolioHoldingRow;
  }
  ...
  return db.query('SELECT * FROM portfolio_holdings WHERE id = ?').get(id) as PortfolioHoldingRow;
}

export function deleteHolding(db: Database, id: string): { deleted: boolean } {
  const result = db.run('DELETE FROM portfolio_holdings WHERE id = ?', [id]);
...
export function getPortfolio(db: Database, input: GetPortfolioInput = {}): PortfolioSummary {
  ...
  const holdings = db
    .query(`SELECT * FROM portfolio_holdings ${where} ORDER BY value DESC NULLS LAST`)
    .all(...params) as PortfolioHoldingRow[];
...
export async function refreshPrices(db: Database, account_id?: string): Promise<PortfolioSummary> {
  ...
  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
  const holdings = db
    .query(`SELECT * FROM portfolio_holdings ${where}`)
    .all(...params) as PortfolioHoldingRow[];
  ...
  for (const holding of holdings) {
    ...
    try {
      const result = await priceRegistry.getPrice(
        holding.symbol,
        assetType,
      ...
      db.run(
        `UPDATE portfolio_holdings
         ...
         WHERE id = ?`,
        [newPrice, newValue, result.source, result.asOf.toISOString(), ts, holding.id]
      );
    } catch {
